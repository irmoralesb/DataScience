...
The students that test well are at the top of the image and there seem to be three groupings by subject. correct
...
What proportion of the total variability is explained by the first three columns of  YV ? 98
...
We see that almost 99% of the variability is explained by the first three columns of  YV=UD . So we get the sense that we should be able to explain much of the variability and structure we found while exploring the data with a few columns.
...
$$
    UD=[U_1d_{1,1},U_2d_{2,2},…,U_{p}d_{p,p}] 
$$  
...
my_image(cor(resid), zlim = c(-1,1))
axis(side = 2, 1:ncol(y), rev(colnames(y)), las = 2)
and see that the structure that is left is driven by the differences between math and science. Confirm this by first plotting  U3 , then plotting  V⊤3 using the same range for the y-axis limits, then making an image of  U3d3,3V⊤3  and comparing it to the image of resid.
