---
title: "Linear Regression - Introduction to Regression"
...
If you hit it hard enough, it's a home run, the best possible outcome
as you get at least one automatic run.
But sometimes, due to chance, you hit the ball very hard
and a defender catches it, which makes it an out, a failure.
In contrast, sometimes you hit the ball softly
but it lands just in the right place.
You get a hit which is a success.
The fact that there is chance involved hints
at why probability models will be involved in all this.
...
from the formula mu_x and mu_y are the averages of the corresponding dataset set. and sigma_X and sigma_y are the standard deviation. 
...
will be positive.
That happens when they are both positive or when they are both negative as often
as they will be negative.
That happens when one is positive and the other
is negative, or the other way around.
...
If instead the quantities vary together, then we
are averaging mostly positive products.
Because they're going to be either positive times
positive or negative times negative.
...
![Correlation Coefficient Formular 1 to 1](figs/correlation_coefficient_formula1.png)

**The correlation between father and sons' height is about 0.5**
**NOTE: The dataset downloaded is getting different results than the ones in the course, **
...
here are six examples of pairs with correlations ranging from negative 0.9
to 0.99.
...
**Reminder about random variability.**
...
![Normal Distribution](figs/normal_distribution.png)
...
a good one, as we see in this ggplot.
...
We start by demonstrating how correlation
...
we know that the average height of 70.5 inches
is a value with the highest proportion and would
...
*The father is taller than average*, specifically
he is 1.14 standard deviations taller than the average father.
So shall we predict that the son is also 1.14 standard deviations taller
than the average son?
...
We call this a *conditional average*, since we
are computing the average son height conditioned on the father
being 72 inches tall.

A challenge when using this approach in practice
is that we don't have many fathers that are exactly 72.
In our data set, we only have eight.
If we change the number to 72.5, we would only
have one father who is that height.
This would result in averages with large standard errors,
...
of fathers with very similar heights.
Specifically, we will round fathers' heights to the nearest inch.
This gives us the following prediction for the son of a father that
is approximately 72 inches tall.
...
Stratification followed by box plots lets
us see the distribution of each group.
Here is that plot.
...
We can see that the centers of these groups
are increasing with height, not surprisingly.
...
**The slope of this line appears to be about 0.5,**
which happens to be the correlation between father and son heights.
This is not a coincidence.
To see this connection, let's plot the standardized heights
against each other, son versus father, with a line that
has a slope equal to the correlation.
Here's the code, Here's a plot.
...
In a later video, we will describe Galton's theoretical justification
for using this line to estimate conditional means.
...
![Regressio Line formula](figs/regression_line_formula.png)
...
In fact, the title of Galton's paper was "Regression Towards Mediocrity
...
We can compute the intercept and the slope
using the formulas we just derived.
...
If we plot the data in standard units, then, as we discussed,
the regression line as intercept 0 and slope rho.
...
We started this discussion by saying that we
wanted to use the conditional means to predict the heights of the sons.
...
An advantage of using the regression line
is that we used all the data to estimate just two parameters, the slope
and the intercept.
...
The main way we motivate the use of correlation
involve what is called the **bivariate normal distribution**.
...
They can be thin.
That's when they have high correlation.
...
of the y's in the group--
defined by setting x in this way--
...
and see that the assumption appears to hold.
...
Galton showed-- using mathematical statistics--
that **when two variables follow a bivariate normal distribution, then for any given x the expected value of the y in pairs for which x is set at that value is mu y plus rho x minus mu x divided by sigma x times sigma y.**

![Expected formula](figs/expected_formula.png)

Note that this is a line with slope rho times
sigma y divided by sigma x 
...
So in summary, if our data is approximately bivariate,
then the conditional expectation -- which is the best prediction for y given that we know the value of x-- is given by the regression line.
...
We used these calculations-- the code where we calculated all values (ln 484)--
